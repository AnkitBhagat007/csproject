...
void addToIssuedBook(int book_number)
{
    ...
    while(1)
    {
        ...
        if(line==book_number)
        {
            strcpy(temp.issueDate,formatIssueDate());
            strcpy(temp.dueDate,formatDueDate(issue_time));
...
{    
    ...
    if(file!=NULL){     // if file exist
        while(1)
        {
            ...
            printf("Book %d:-%s   Author:-%s   Due Date:- %s  Issue Date:- %0.10s",i,temp.name,temp.author,temp.dueDate,temp.issueDate);
            printf("\n\n");
            
            i++;
        }

        printf("Total Books issued to you :- %d\n",number_of_books());
